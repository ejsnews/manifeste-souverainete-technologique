---
layout: default
lang: fr
title: "COMPARAISON MONDIALE DES SOURCES D'ÉNERGIE : POURQUOI LA THERMOLYSE HAFFNER ENERGY CHANGE TOUT"
---
# COMPARAISON MONDIALE DES SOURCES D'ÉNERGIE : POURQUOI LA THERMOLYSE HAFFNER ENERGY CHANGE TOUT [(English version - EN)](ENERGY_SOURCES_COMPARISON_WHY_HAFFNER_ENERGY_THERMOLYSIS_CHANGES_EVERYTHING.md)

📥 [Consulter le Manifeste pour la souveraineté et la résilience technologique](../README.md)

📥 [Classement climatique des sources d'énergies](CLASSEMENT_CLIMATIQUE_SOURCES_ENERGIE_CHALEUR_ANTHROPIQUE.md)

---

*Note technique et économique indépendante — EJS — Juin 2026*

---

## Introduction : l'énergie, un choix de civilisation

Face à la capitulation des États-Unis face à l'Iran, le choix d'une source d'énergie ne se résume pas à choisir un prix par kilowattheure. C'est choisir un niveau de dépendance géopolitique, une empreinte sur les sols et l'atmosphère, une capacité de résilience en cas de crise, et une trajectoire pour les générations suivantes.

Le tableau ci-dessous compare l'ensemble des sources 
d'énergie disponibles ou en développement selon 
neuf critères objectifs, dans le contexte français 
et européen de 2026. Il ne s'agit pas d'un plaidoyer 
idéologique : les chiffres parlent d'eux-mêmes.

Une technologie se distingue de toutes les autres 
par sa capacité à satisfaire simultanément 
l'ensemble des critères : **la thermolyse décentralisée 
Haffner Energy**.

---

## Le tableau comparatif

| Source d'Énergie | Souveraineté | Non-carbonisé | Décarbonise | Pol. Air | Pol. Sols | CO₂ | Prix (€/kWh) | CAPEX (€/kW) |
|---|---|---|---|---|---|---|---|---|
| **Haffner H6/C-iC biochar** | **🟢 Nul** | **✅** | **✅** | **🟢 Nulle** | **🟢 Régénère** | **🟢** | **< 0,060** | **1 000** |
| **Haffner H4/S-iC biochar (à venir)** | **🟢 Nul** | **✅** | **✅** | **🟢 Nulle** | **🟢 Régénère** | **🟢** | **< 0,030** | **En cours** |
| Charbon | 🔴 Élevée | ❌ | ❌ | 🔴 Élevée | 🔴 Élevée | 🔴 | 0,11 | 3 000 |
| Pétrole | 🔴 Élevée | ❌ | ❌ | 🔴 Élevée | 🔴 Élevée | 🔴 | 0,20 | 1 200 |
| Gaz Naturel (CCGT) | 🔴 Élevée | ❌ | ❌ | 🟠 Moyenne | 🟠 Faible | 🔴 | 0,12 | 850 |
| Hydrogène Gris | 🔴 Élevée | ❌ | ❌ | 🟠 Moyenne | 🟠 Faible | 🔴 | 0,085 | 400 |
| Hydrogène Bleu | 🔴 Élevée | ⚠️ | ❌ | 🟠 Moyenne | 🟠 Faible | 🟠 | 0,075 | 600 |
| Fuel Oil (secours) | 🔴 Élevée | ❌ | ❌ | 🔴 Élevée | 🔴 Élevée | 🔴 | 0,22 | 500 |
| Charbon Propre (CCS) | 🔴 Élevée | ⚠️ | ⚠️ | 🟠 Moyenne | 🟠 Faible | 🟠 | 0,16 | 4 500 |
| Nucléaire (parc existant amorti) | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | ⚠️ Déchets | 🟢 | 0,045 | amorti |
| Nucléaire (EPR2 neuf) | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | ⚠️ Déchets | 🟢 | 0,12 | 9 000 |
| Nucléaire Fusion (ITER) | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | 🟢 Nulle | 🟢 | Inconnu | Inconnu |
| Éolien Terrestre | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | 🟢 Nulle | 🟢 | 0,065 | 1 450 |
| Éolien Offshore | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | 🟢 Nulle | 🟢 | 0,090 | 3 200 |
| Solaire PV | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | 🟢 Nulle | 🟢 | 0,050 | 800 |
| Solaire Thermique (CSP) | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | 🟢 Nulle | 🟢 | 0,080 | 4 500 |
| Hydraulique | 🟢 Nul | ✅ | ❌ | 🟢 Nulle | 🟠 Réservoirs | 🟢 | 0,030 | 2 500 |
| Géothermie | 🟢 Nul | ✅ | ❌ | 🟢 Nulle | 🟠 Faible | 🟢 | 0,060 | 3 000 |
| Marémotrice / Houlomotrice | 🟢 Nul | ✅ | ❌ | 🟢 Nulle | 🟢 Nulle | 🟢 | 0,120 | 4 000 |
| Hydrogène Blanc (naturel) | 🟠 Modéré | ✅ | ❌ | 🟢 Nulle | 🟠 Risque | 🟢 | 0,045 | Variable |
| Hydrogène Vert (électrolyse) | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | 🟢 Nulle | 🟢 | 0,18 | 1 800 |
| Biomasse Classique | 🟢 Nul | ⚠️ | ❌ | 🟠 Moyenne | 🟢 Nulle | ⚠️ | 0,10 | 2 200 |
| Biogaz / Méthanisation | 🟢 Nul | ⚠️ | ❌ | 🟠 Faible | 🟢 Nulle | ⚠️ | 0,09 | 2 500 |
| Biocarburants 1G (colza/maïs) | 🟢 Nul | ⚠️ | ❌ | 🟠 Faible | 🟠 Intrants | ⚠️ | 0,14 | 1 500 |
| e-SAF (électrolyse + FT) | 🟡 Faible | ✅ | ❌ | 🟢 Nulle | 🟢 Nulle | 🟢 | 0,30 | 3 500 |

---

## Lecture du tableau

### Les colonnes

**Souveraineté** : le risque géopolitique lié 
à la dépendance aux importations. 🔴 signifie 
une dépendance critique aux marchés mondiaux 
(OPEP, Russie, Chine pour les matériaux). 
🟢 signifie une production entièrement locale 
sur ressources fatales non-conflictuelles.

**Non-carbonisé** : la source ne produit pas 
de CO₂ lors de son fonctionnement. ⚠️ signifie 
une neutralité partielle ou un cycle court 
mais non nul.

**Décarbonise** : la source va au-delà 
de la neutralité carbone — elle séquestre 
activement du CO₂ de l'atmosphère. 
Seul Haffner Energy atteint ce niveau 
grâce au biochar (puits de carbone 
solide et permanent).

**Pol. Air** : émissions de particules fines, 
NOx, SOx lors de la production. 
Le charbon et le pétrole sont les plus 
nocifs pour la santé publique — 
leur coût réel intégrant les pathologies 
respiratoires et cardiovasculaires 
dépasse largement leur prix facial.

**Pol. Sols** : impact sur les terres 
(extraction minière, stockage de déchets, 
risques de fuite). "Régénère" pour Haffner 
signifie que le biochar améliore activement 
la structure et la fertilité des sols.

**Prix (€/kWh)** : coût de revient 
de production, hors subventions, 
en contexte français 2026. 
Pour Haffner H6, la ligne "net + biochar" 
intègre le crédit des co-produits 
(200 kg de biochar à 275-420 €/t 
par tonne de biomasse traitée).

**CAPEX (€/kW)** : investissement 
initial par kilowatt installé. 
Le nucléaire EPR2 neuf (9 000 €/kW) 
illustre le paradoxe français : 
la technologie la plus chère à construire, 
avec les délais les plus longs (15-20 ans), 
pour un résultat centralisé et non souverain 
sur les matériaux (uranium importé).

---

## Ce que le tableau révèle

### Aucune source conventionnelle ne satisfait tous les critères simultanément

Le nucléaire existant est compétitif 
sur le prix mais ne décarbonise pas, 
génère des déchets radioactifs 
sans solution de stockage définitive 
en France, et dépend de l'uranium importé.

L'éolien et le solaire sont propres 
mais intermittents, ne décarbonisent pas, 
et dépendent de matériaux critiques 
(terres rares, silicium) dont la chaîne 
d'approvisionnement est largement contrôlée 
par la Chine.

L'hydrogène vert par électrolyse 
est propre mais coûte trois fois plus 
cher que la production Haffner H6 
(0,18 €/kWh vs < 0,060 €/kWh), 
consomme une électricité précieuse 
et concourt avec les usages industriels 
et numériques sur un réseau déjà saturé.

La biomasse classique et la méthanisation 
émettent des particules, ne séquestrent 
pas le carbone, et nécessitent 
des infrastructures lourdes.

### Haffner H6 : la seule source à case verte sur toute la ligne

C'est le seul système qui combine 
simultanément :
- Souveraineté totale (biomasse résiduelle locale)
- Zéro émission de particules
- Décarbonisation active (biochar)
- Prix compétitif avec le fossile 
  dès le module actuel H6
- CAPEX accessible (1 000 €/kW, 
  finançable en leasing)
- Déploiement rapide (< 1 mois, 
  sans génie civil)
- Régénération des sols (biochar 
  comme amendement agricole)


### Le module H4/S-iC : une rupture supplémentaire en préparation

Le module H6 (C-iC) est la technologie 
disponible aujourd'hui. Haffner Energy 
développe en parallèle un second module, 
le H4 (S-iC), dont les données 
de performance n'ont pas encore 
été communiquées au marché.

Ce qu'on peut dire publiquement : 
la trajectoire technologique observée 
sur le H6 (baisse continue du coût 
de production) se poursuit avec le H4, 
qui vise un rendement de production 
significativement supérieur 
pour un coût unitaire sensiblement 
inférieur. Lorsque ces données 
seront officiellement publiées, 
elles repositionneront Haffner Energy 
comme la source d'énergie 
la moins chère du tableau — 
toutes catégories confondues, 
fossiles inclus.

La case "En cours" du tableau 
n'est pas un aveu d'incertitude : 
c'est une promesse en cours 
de validation industrielle.

---

## Conclusion : arrêter de subventionner la mauvaise colonne

Les politiques énergétiques françaises 
et européennes subventionnent massivement 
des technologies qui n'obtiennent pas 
la case verte sur toute la ligne : 
l'éolien offshore (3 200 €/kW, 
intermittent), l'e-SAF (0,30 €/kWh, 
dépendant de l'électricité), 
l'hydrogène vert par électrolyse 
(0,18 €/kWh, concurrence les réseaux).

Pendant ce temps, la seule technologie 
qui coche toutes les cases — 
production, décarbonisation, souveraineté, 
prix, sols, santé publique — 
attend un fast-track réglementaire 
que la bureaucratie française 
n'a pas encore accordé.

Le Maharashtra, lui, n'a pas attendu.

---

*Note technique et économique indépendante — EJS — Juin 2026.
L'auteur est actionnaire à titre individuel
et n'agit pas pour le compte de la société citée.
Ce texte ne constitue pas un conseil en investissement.
Les données chiffrées sont issues de sources publiques
(ADEME, AIE, IRENA, Haffner Energy communications officielles)
et de calculs indépendants basés sur des données publiques.*
