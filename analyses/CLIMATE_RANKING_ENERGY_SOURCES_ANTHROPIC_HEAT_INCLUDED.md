---
layout: default
lang: en
title: "CLIMATE RANKING OF ENERGY SOURCES"
---
# CLIMATE RANKING OF ENERGY SOURCES [(French version - FR)](CLASSEMENT_CLIMATIQUE_SOURCES_ENERGIE_CHALEUR_ANTHROPIQUE.md)

## Scoring out of 24 points — Anthropic Heat Included

📥 [Consult the Manifesto for Technological Sovereignty and Resilience](../README.md)

📥 [Global comparison of energy sources: HAFFNER ENERGY changes everything](ENERGY_SOURCES_COMPARISON_WHY_HAFFNER_ENERGY_THERMOLYSIS_CHANGES_EVERYTHING.md)

📥 [Hydrogen independence within reach of every region, community, and rural elected official](HYDROGEN_INDEPENDENCE_WITHIN_REACH_OF_EVERY_TERRITORY_LOCAL_AUTHORITIES_RURAL_ELECTED_AUTHORITIES.md)

*Independent Technical Note — EJS — June 2026*

---
### Methodology — 6 criteria × 4 points max = 24 pts

| Criterion              | Weighting | Description |
|------------------------|-----------|-----------|
| 🏭 **Sovereignty**     | /4        | Geopolitical dependence on imports |
| ♻️ **Non-CO₂**         | /4        | Zero CO₂ emissions in operation |
| 🌱 **Decarbonizes**    | /4        | Actively sequesters existing atmospheric CO₂ |
| 💨 **Air**             | /4        | Zero fine particles, NOx, SOx |
| 🌍 **Soils**           | /4        | Zero or positive impact on soils and water |
| 🌡️ **Net Heat**        | /4        | Zero anthropic heat added to the atmosphere |

> **Note on anthropic heat:** Any stock energy (fossil, nuclear) adds net heat to the Earth's envelope, independently of CO₂.  
> Flow energies (solar, wind, hydropower) intercept energy already present in the natural cycle — they do not add net heat.  
> Haffner Energy is the only system that **reduces** dissipated heat by sequestering the carbon that was blocking its evacuation into space.

---
<table class="ranking-table">
<thead>
<tr>
  <th>#</th>
  <th>Energy Source</th>
  <th>🏭 Sov.</th>
  <th>♻️ CO₂</th>
  <th>🌱 Decarb.</th>
  <th>💨 Air</th>
  <th>🌍 Soils</th>
  <th>🌡️ Heat</th>
  <th>📊 /24</th>
</tr>
</thead>
<tbody>
<tr class="r-s1">
  <td>1</td><td>🇫🇷 Haffner H4/S-iC ⏳</td>
  <td>4</td><td>4</td><td>6★</td><td>4</td><td>4</td><td>4★</td>
  <td class="score">24+★⏳</td>
</tr>
<tr class="r-s3">
  <td>2</td><td>🇫🇷 Haffner H6/C-iC (+ biochar)</td>
  <td>4</td><td>4</td><td>6★</td><td>4</td><td>4</td><td>4★</td>
  <td class="score">24+★</td>
</tr>
<tr class="r-s4">
  <td>3</td><td>Tidal / Wave</td>
  <td>4</td><td>4</td><td>0</td><td>4</td><td>4</td><td>4</td>
  <td class="score">18</td>
</tr>
<tr class="r-s5">
  <td>4</td><td>Onshore Wind</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>4</td><td>4</td>
  <td class="score">17</td>
</tr>
<tr class="r-s6">
  <td>5</td><td>Offshore Wind</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>3</td><td>4</td>
  <td class="score">16</td>
</tr>
<tr class="r-s6">
  <td>6</td><td>Solar PV</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>3</td><td>4</td>
  <td class="score">16</td>
</tr>
<tr class="r-s6">
  <td>7</td><td>Solar CSP</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>3</td><td>4</td>
  <td class="score">16</td>
</tr>
<tr class="r-s6">
  <td>8</td><td>Hydropower</td>
  <td>4</td><td>4</td><td>0</td><td>4</td><td>3</td><td>4</td>
  <td class="score">16</td>
</tr>
<tr class="r-s7">
  <td>9</td><td>Geothermal</td>
  <td>4</td><td>4</td><td>0</td><td>4</td><td>3</td><td>3</td>
  <td class="score">15</td>
</tr>
<tr class="r-s7">
  <td>10</td><td>White Hydrogen (natural)</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>3</td><td>3</td>
  <td class="score">15</td>
</tr>
<tr class="r-s7">
  <td>11</td><td>Nuclear Fusion (ITER)</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>4</td><td>2</td>
  <td class="score">15</td>
</tr>
<tr class="r-s8">
  <td>12</td><td>Biogas / Methanization</td>
  <td>4</td><td>2</td><td>1</td><td>3</td><td>3</td><td>3</td>
  <td class="score">13</td>
</tr>
<tr class="r-s8">
  <td>13</td><td>Green Hydrogen (electrolysis)</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>3</td><td>1</td>
  <td class="score">13</td>
</tr>
<tr class="r-s9">
  <td>14</td><td>Conventional Biomass</td>
  <td>4</td><td>2</td><td>1</td><td>2</td><td>3</td><td>3</td>
  <td class="score">12</td>
</tr>
<tr class="r-s9">
  <td>15</td><td>e-SAF (electrolysis + FT)</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>3</td><td>1</td>
  <td class="score">12</td>
</tr>
<tr class="r-s9">
  <td>16</td><td>Nuclear (existing amortized fleet)</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>2</td><td>1</td>
  <td class="score">12</td>
</tr>
<tr class="r-s9">
  <td>17</td><td>New EPR2 Nuclear</td>
  <td>3</td><td>4</td><td>0</td><td>4</td><td>2</td><td>1</td>
  <td class="score">12</td>
</tr>
<tr class="r-s10">
  <td>18</td><td>Clean Coal (CCS)</td>
  <td>2</td><td>2</td><td>2</td><td>2</td><td>2</td><td>1</td>
  <td class="score">10</td>
</tr>
<tr class="r-s11">
  <td>19</td><td>1G Biofuels (rapeseed/maize)</td>
  <td>4</td><td>2</td><td>0</td><td>2</td><td>2</td><td>2</td>
  <td class="score">9</td>
</tr>
<tr class="r-s12">
  <td>20</td><td>Blue Hydrogen</td>
  <td>2</td><td>2</td><td>0</td><td>2</td><td>2</td><td>1</td>
  <td class="score">7</td>
</tr>
<tr class="r-s13">
  <td>21</td><td>Natural Gas (CCGT)</td>
  <td>2</td><td>1</td><td>0</td><td>2</td><td>2</td><td>0</td>
  <td class="score">6</td>
</tr>
<tr class="r-s13">
  <td>22</td><td>Grey Hydrogen</td>
  <td>2</td><td>1</td><td>0</td><td>2</td><td>2</td><td>0</td>
  <td class="score">6</td>
</tr>
<tr class="r-s14">
  <td>23</td><td>Fuel Oil (backup)</td>
  <td>1</td><td>0</td><td>0</td><td>1</td><td>1</td><td>0</td>
  <td class="score">3</td>
</tr>
<tr class="r-s14">
  <td>24</td><td>Oil</td>
  <td>1</td><td>0</td><td>0</td><td>1</td><td>1</td><td>0</td>
  <td class="score">3</td>
</tr>
<tr class="r-s15">
  <td>25</td><td>Coal</td>
  <td>1</td><td>0</td><td>0</td><td>0</td><td>1</td><td>0</td>
  <td class="score">1</td>
</tr>
</tbody>
</table>

[HYNOCA® in the spotlight: REFORMERS EU-funded Renewable Energy Valley project in the Netherlands wins the « Clean Project » award at World Hydrogen 2025](https://www.haffner-energy.com/hynoca-recognized-as-eu-funded-reformers-renewable-energy-valley-project-in-netherlands-awarded-world-hydrogen-2025-prize-clean-project-category/)

![Haffner Energy is the winner and game changer](images/haffner_winner_and_game_changer_EN.jpg)

---
### Legend

| Symbol     | Meaning |
|------------|---------|
| ★          | Exceeds theoretical maximum — net positive effect on climate |
| ⏳         | Performance data not yet disclosed to the market |
| 🇫🇷         | French technology |

### What This Ranking Reveals

Conventional renewable energies top out at **15-18/24** — they do not decarbonize; they only limit new emissions.

**Nuclear drops to 12/24** despite its excellence on CO₂: the fatal heat dissipated into rivers (2 GW thermal for 1 GW electric across a fleet of 56 reactors) and radioactive waste with no permanent solution in France heavily penalize the score.

**Haffner H6 exceeds the 24-point maximum** — the only system with a net positive effect on the climate, which sequesters existing carbon instead of simply avoiding new emissions.

**The H4/S-iC**, currently under development, aims for the same level with significantly higher efficiency and a substantially lower unit cost — data not yet disclosed to the market.

---
*Independent Technical Note — EJS — June 2026.*  
*The author is an individual shareholder and does not act on behalf of the company mentioned.*  
*This text does not constitute investment advice.*
